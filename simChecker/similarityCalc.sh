...
do
	...
	echo $alljnkname |grep -v 'job[1-5]CP'>> columns.all.list
	echo $usedjnkname |grep -v 'job[1-5]CP'>> columns.used.list
...
cat fittness.list |grep -v 'job[1-5]CP'| sort -nr -k2 | awk '{print $1}' > columns.all.list.ordered
cat fittness.list | grep -v 'job[1-5]CP'|sort -nr -k2 | awk '{print $1}'| sed 's/all/used/g'  > columns.used.list.ordered
...
if [ ! -e simMatrix ]; then
	...
	if [ "$simVsBest" == 1 ]; then
		...
		#     ./simMatrix -l columns.used.list -l > similarityMatrix.used.dat
		echo "calculating the similarity matrix for the used and all reactions lower and upper diagonals"
			./simMatrix -l columns.all.list -u columns.used.list > similarityMatrix.dat
	fi
fi
